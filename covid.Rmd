...
Uma variável aleatória $X$, como definica anteriormente, segue uma \textbf{distribuição de Bernoulli} com parâmetro $0\leq p\leq 1$ se a sua função de probabilidade é dada por
...
- Uma outra característica que surge imediatamente no caso da COVID-19 é os dados de {\it contagem}. Uma distribuição geralmente utilizada para descrever a aleatoriedade deste tipo de variável é a {\bf distribuição de Poisson}.
...
- Por fim, apresentamos a {\bf distribuição Binomial} que descreve a aleatoriedade para variáveis aleatórias com a seguinte características: contar o número de sucessos em $y$ ensaios independentes de Bernoulli com parâmetro $p$. Assim, obteremos a distribuição Binomial com parâmetros $y$ e $p$.
...
**Esperança e Variânça:** Se $Z \sim B(y, p)$, então:
\begin{eqnarray*}
E (Z) &=& yp\\
Var(Z) &=& yp (1 - p)
\end{eqnarray*}
...
- Como citado nos exemplos anteriores, podemos supor que há um número {\bf verdadeiro} de casos que ocorreram em um município, digamos $Y$, mas que apenas $Z$ casos são notificados às autoridades. Uma restrição que surge em $Z$ é $$Z=0, 1, ..., Y \;\;\;\text{ em que }\;\;\; Y=0, 1, 2, 3, ...$$

- Dessa forma, consideramos que $Y\sim Poi(\lambda)$ e que representa o número verdadeiro de casos e supondo que a {\bf probabilidade de notificação} é $p$ em todos os casos, então o número de casos notificados, dado o número verdadeiro, é $Z|y \sim Bin(y, p)$, ou seja, obtemos o {\bf modelo hierárquico Poisson-Binomial}:
...
- Como comentado, as ferramentas para relacionar um conjunto de características chamado de {\bf variáveis explicativas} (VE) com uma variável de interesse, chamada de {\bf variável resposta} (VR) são os {\bf modelos de regressão}. 
...
**Função de ligação:** Como $\mu>0$, a função identidade não é adequada pois o preditor linear pode ser um número real, enquanto $\mu>0$. Assim, uma escolha é $g(\mu) = \log(\mu)\in \mathbb{R}$ e assim obteremos um modelo chamado de {\bf log-linear}: 
\begin{eqnarray*}
log (\mu) = \alpha + \beta x
\end{eqnarray*}
